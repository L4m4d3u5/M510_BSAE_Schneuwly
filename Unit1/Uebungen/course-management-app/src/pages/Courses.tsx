...
const Courses: React.FC = (): React.ReactElement => {
    ...
    const cycleStatus = () => {
        setStatusFilter((prev) => (prev === null ? CourseStatus.ACTIVE : prev === CourseStatus.ACTIVE ? CourseStatus.INACTIVE : null));
    };
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                ...
                >
                    {statusFilter === null ? 'nach status filtern' : statusFilter === CourseStatus.ACTIVE ? 'Aktiv' : 'Inaktiv'}
                </button>
            </div>
                        ...
                        {filteredCourses.map((course) => (
                            <tr key={course.id}>
                                ...
                                <td>
                                    <span
                                        className={
                                            course.status === CourseStatus.ACTIVE ? 'badge badge-active' : 'badge badge-inactive'
                                        }
                                    >
